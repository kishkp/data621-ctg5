...
# 
kable(eval_ds_TA[1:10,c(1,2,61,3)], caption="Outcome on evaluation data set")
...
Outcome from regression model and outcome from linear model was plotted in the chart above.It can be seen from the chart above that probability associated with classification and predicted amount from linear model are linearly related though those two outcome came from two ompletely different models. Relationship between those two are linear which indicates with the increase in probabilty for accident outcome, expense amount for repair also goes up with given data set. One key reason for that is there are number of variables in given data set which is common and playing important roles both the models such as- URBANICITY_Rural, MSTATUS_Yes,  PARENT1_Yes, REVOKED_Yes,TRAVTIME,KIDSDRIV. \ 
