...
##### Use of step function to enhance the model1created above
```{r model 1 enhancment,echo = FALSE, warning=FALSE, message=FALSE}
...
##### Interpretation for model 1 and enhanced model_ref  \


**NEED TO WORK ON THIS**

$\newline$
(i) model 1 has AIC value 6078.7 and enhaned model has AIC value 6064.9 slightly better compared to model1. we will look into more details on model1_ref below-

(i) Based on the outcome from model_ref, it can be seen that following variables KIDSDRIV,PARENT1_Yes, MSTATUS_Yes,CAR_USE_Commercial, REVOKED_Yes, TIF_sin , CAR_TYPE_FLAG_BIN ,EDUCATION_FLAG_BIN,JOB_TYPE_FLAG_BIN,INCOME_FLAG_BIN ,HOME_VAL_FLAG_BIN,OLDCLAIM_FLAG_BIN,MVR_PTS_FLAG_BIN,TRAVTIME_FLAG_BIN ,URBANICITY_Rural are only statistically significant. 

(ii)As for the statistically significant variables, URBANICITY_Rural has the lowest p-value suggesting a strong association of the URBANICITY_Rural  to the target variable.
...
(iii) The logistic regression coefficients give the change in the log odds of the outcome for a one unit increase in the predictor variables.
  ...
  a. For change in KIDSDRIV variable when kids are in car, the log odds of crime rate above median value increases by 0.41327 .\
  b. When the driver is a Parent then with change in PARENT1_Yes, the log odds of incident value increases by 0.48010\
  c. with change in marital status the log odds of accident decreases by  -0.44485.\
  d. With change in status of variable URBANICITY_Rural, the log odds of accident decreases by -2.20258. 
...
#####  Interpretation for model 2 using CRT and pruning to optimal model2_ref  \

$\newline$

(i) Based on the model 2 output following variables have been used for classification-CAR_TYPE_FLAG_BIN,CAR_USE_Commercial,JOB_TYPE_FLAG_BIN, KIDSDRIV ,OLDCLAIM_FLAG_BIN, REVOKED_Yes, URBANICITY_Rural

(ii) lowest Cp value and Xerror occured on split 7


(iii) after pruning also no of optimal split was 7
...
# Prediction using the model 3
 prediction_model3<-data.frame(predict(model3,newdata=DS_TARGET_FLAG_VALID))

table(DS_TARGET_FLAG_VALID$TARGET_FLAG,prediction_model3$posterior.1>0.5)
...
(i)The Classification boundary equation for our model 3 is below: \

 $0.359 * KIDSDRIV  + 0.025 * HOMEKIDS  -0.003 * CAR_AGE + 0.472 * PARENT1_Yes  -0.360 * MSTATUS_Yes + 0.036 *SEX_M + 0.551 * CAR_USE_Commercial -0.030* RED_CAR_yes -  0.832 * REVOKED_Yes +  0.137 * TIF_sin   - -0.027 * BLUEBOOK_sin + 0.012 * AGE_sin -0.544 * CAR_TYPE_FLAG_BIN --0.382*EDUCATION_FLAG_BIN- 0.503*JOB_TYPE_FLAG_BIN + 0.657 *INCOME_FLAG_BIN+ 0.657 *INCOME_FLAG_BIN -0.133 *YOJ_FLAG_BIN+ 0.234* HOME_VAL_FLAG_BIN - -0.271 * OLDCLAIM_FLAG_BIN -0.271* CLM_FREQ_FLAG_BIN -0.263 * MVR_PTS_FLAG_BIN 0.005 * CAR_AGE_FLAG_BIN -0.359 *TRAVTIME_FLAG_BIN -1.529 * URBANICITY_Rural



(ii) From summary table, we also have the prior probability of of not having accident incident in data set is 0.7350821 and failure is 0.2649179.\

(iii) Group means provides mean values for each variable with respect to target variable values 0 and 1.\

 (iv) LDA model assumes normality of the variables used in the model and there are some variables which are not completely normally distributed and have outliers that is impacting the result of this model.
...
###2.4.1 Model Evaluation Using 20% evaluation Data 

#### Evaluation of model1 
```{r model1 evalution,echo = FALSE, warning=FALSE, message=FALSE}

#prediction based on model1_ref
prediction_model1<-predict(model1,newdata=DS_TARGET_FLAG_VALID)

table(DS_TARGET_FLAG_VALID$TARGET_FLAG,prediction_model1>0.5)

```


#### Evaluation of model1_ref
```{r model1_ref evalution,echo = FALSE, warning=FALSE, message=FALSE}

#prediction based on model1_ref
prediction_model1<-predict(model1_ref,newdata=DS_TARGET_FLAG_VALID)
...
#prediction based on model1_ref
prediction_model2<-data.frame(predict(model2_ref,newdata=DS_TARGET_FLAG_VALID))

table(DS_TARGET_FLAG_VALID$TARGET_FLAG,prediction_model2$X1>0.5)
...
m1<-lm(TARGET_AMT~., na.omit(DS_TARGET_AMT_TRAIN))

m2<-lm(TARGET_AMT~., na.omit(x))

par(mfrow=c(1,1))
plot(DS_TARGET_AMT_TRAIN$CAR_AGE_AMT_BIN,DS_TARGET_AMT_TRAIN$TARGET_AMT)
