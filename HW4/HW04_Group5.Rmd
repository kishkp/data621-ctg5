...
if (!require("dummy",character.only = TRUE)) (install.packages("dummy",repos = "http://cran.us.r-project.org", dep=TRUE))
if (!require("reshape2",character.only = TRUE)) (install.packages("reshape2",repos = "http://cran.us.r-project.org", dep=TRUE))
if (!require("popbio",character.only = TRUE)) (install.packages("popbio",repos = "http://cran.us.r-project.org", dep=TRUE))
if (!require("rpart",character.only = TRUE)) (install.packages("rpart",repos = "http://cran.us.r-project.org", dep=TRUE))
if (!require("pROC)",character.only = TRUE)) (install.packages("pROC)",repos = "http://cran.us.r-project.org", dep=TRUE))
library(ggplot2)
library(MASS)
library(knitr)
library(xtable)
...
1. TARGET_FLAG - This dependent variable tells whether there was a crash or not. This is a binary variable and as such we will be using a Logistic Regression Model.
2. TARGET_AMT - This dependent variable gives the amount / cost of repairs if there was a crash. This is a continuous variable and we will be using a Linear Regression.
...
pander::pander(variables, split.cells = c(60, 20, 30), split.table = Inf, justify = 'left', caption = "Variable Description")
...
In addition, from the summary output, some numeric variables like INCOME, HOME_VAL, BLUEBOOK, OLDCLAIM have been converted to Factor variables which need to be rectified. \
In addition, there are records where CAR_AGE is negative or zero, which is improbable. Upon investigation, we find that there are 4 records that are affected. We will remove these records.
...
In this section we will use Logistic regression to model the TARGET_FLAG. We will first start with the Data Summary and Correlation.
...
(iv) variable REVOKED_Yes has strong association which indicates if person's license has been revoked in last 7 years then chane of end up in accidents are much higher with log odds value of 1.052. 

(v) If person has a claim in last 5 years then chances of more claims are higher. Variable OLDCLAIM_FLAG_BI indicates that with negative log odds value(1 is here no claim  -.4899). 

(vi) AIC value of the model is AIC: 5962.1 and number of iteration was 5. 
...
Model 1 has AIC value 5962.1 and enhaned model TF_Model1_ref has same AIC value  5962.1. We will look into more details on model1_ref below-

(i) Based on the outcome from model_ref, it can be seen that following variables KIDSDRIV,PARENT1_Yes, \
MSTATUS_Yes,CAR_USE_Commercial, REVOKED_Yes, TIF_sin , CAR_TYPE_FLAG_BIN , EDUCATION_FLAG_BIN, JOB_TYPE_FLAG_BIN, \
INCOME_FLAG_BIN, HOME_VAL_FLAG_BIN,OLDCLAIM_FLAG_BIN,MVR_PTS_FLAG_BIN, \
TRAVTIME_FLAG_BIN , URBANICITY_Rural are only statistically significant. \ 
Most of the variables are having similar association as above model 1. \
...
(iii) One interesting outcome is when childrens are dirivng your car then more chances of accidents with log odd value of 0.3979 for variable KIDSDRIV. 

  (iv) For variable CAR_TYPE_FLAG_BIN there is high negative corelation is there with log odds value of -0.5584 that means Mnivan and Panel truck has higher chance of getting into an accident. 
  
  (v) Variable EDUCATION_FLAG_BIN has negative log odds value of -0.3764 indicaating that people with higher education above high school has less chance of an accident comared to the other group. 
...
***Pruned Classification Tree for TARGET_FLAG**
\
...
CAR_USE_Commercial, REVOKED_Yes, URBANICITY_Rural, CAR_TYPE_FLAG_BIN,TRAVTIME, OLDCLAIM_FLAG_BIN, MVR_PTS, TIF_SIN, KIDSDRIV, PARENT1_Yes, EDUCATION_FLAG_BIN, MSTATUS_Yes, BLUEBOOK, OLDCLAIM, JOB_TYPE_FLAG_BIN, HOME_VAL, INCOME_FLAG_BIN. 
...
#df3<- data.frame(score, step4.res)
#ggplot(df3, aes(score, step4.res)) +
 # geom_point() +
#  geom_smooth()+
 # ggtitle("Residual vs Predicted values ") +
  #labs(x="Predicted Cost of Repair",y="Residuals") 
```
...
ggplot(Class_Expense, aes(TARGET_FLAG_prob, TARGET_AMT)) +
  geom_point() +
  geom_smooth()+
  ggtitle("Amount Paid vs Probability of Crash ") +
  labs(x="Probability of Crash",y="Amount Paid") 
```

The outcome from the logistical regression model and the linear regression model was plotted in the chart above. It can be seen that probability associated with the classification and the predicted amount from linear model does not show any specific patterns. From insurance business perspective, cases where the probability of incident and the repair expense amount are high will be the focus area; which is the top right side corner of the chart above.
