...
- Model 1 has AIC value 4713 after backward selection. -
...
- The logistic regression coefficients give the change in the log odds of the outcome for a one unit increase in the predictor variables. For Example:
  a. For change in KIDSDRIV variable when kids are in car, the log odds of crime rate above median value increases by 0.34310 .\
  b. When the driver is a Single Parent the log odds of an accident increases by 0.52460\
  c. with change in marital status the log odds of accident decreases by 0.40660.\
  d. If the CAR_TYPE is a MiniVan or a Panel Truck the log odds of accident decreases by 0.56620. \
...
- The following variables have been used for classification - OLD_CLAIM, JOB_TYPE_FLAG_BIN, URBANICITY_Rural, MVR_PTS, REVOKED_Yes, Home_VAL.\
\

- lowest Cp value and Xerror occured on split 7
\
\
...
TF_Model3=lda(TARGET_FLAG~.,data=ds)
TF_Model3
```
...
$ 0.2862 * KIDSDRIV + 0.0232 * HOMEKIDS - 0.0021 * CAR_AGE + 0.5721 * CAR_USE_Commercial - 0.2925 * MSTATUS_Yes + 0.5607 * PARENT1_Yes - 0.1136 * RED_CAR_yes + 0.8313 * REVOKED_Yes + 0.0805 * SEX_M - 1.4872 * URBANICITY_Rural + 0.137 * TIF_sin - 0.005 * BLUEBOOK_sin + 0.0119 * AGE_sin - 0.5499 * CAR_TYPE_FLAG_BIN - 0.3787 * EDUCATION_FLAG_BIN + 0.5154 * JOB_TYPE_FLAG_BIN + 2.2199 * INCOME_FLAG_BIN - 1.7828 * YOJ_FLAG_BIN + 0.2537 * HOME_VAL_FLAG_BIN - 0.2888 * OLDCLAIM_FLAG_BIN - 0.2888 * CLM_FREQ_FLAG_BIN - 0.262 * MVR_PTS_FLAG_BIN + 0.0723 * CAR_AGE_FLAG_BIN - 0.3941 * TRAVTIME_FLAG_BIN $

- From summary table, we also have the prior probability of of not having accident incident in data set is 0.733993 and failure is 0.266007.\
...
ds$M3_TARGET_FLAG <- as.integer(unlist(data.frame(predict(TF_Model3,newdata=ds))[,1]))
df_pre_train1<-as.data.frame(table(ds$M3_TARGET_FLAG==1,ds$TARGET_FLAG))
df_pre_train1$metrics <- paste(df_pre_train1$Var1,df_pre_train1$Var2, sep = '_') 
...
###2.4.3 Final Model Selection Summary
...
##4.2.1 Detailed Inference for Final Model
...
##4.2.4 ROC curve for the selected model
```{r,echo=FALSE}
#AUC
...
# Put all Transform statements here
...
TARGET_FLAG_prob <- predict(model1, type ="response", newdata=eval_ds)
...
eval_ds_TA <- eval_ds[eval_ds$TARGET_FLAG==1,]
...
eval_ds_TA$TARGET_AMT <- predict(TA_model1, type ="response", newdata=eval_ds_TA)
