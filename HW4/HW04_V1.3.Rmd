...
-Model 2- This model will be created using calssification and regression tree. \
...
##### Interpretation for the First Model  \

$\newline$


- Model 1 has AIC value 5934.6 after backward selection. -
- Based on the outcome from model_ref, it can be seen that following variables CAR_USE_Commercial, REVOKED_Yes, URBANICITY_Rural, CAR_TYPE_FLAG_BIN,TRAVTIME, OLDCLAIM_FLAG_BIN, MVR_PTS, TIF, KIDSDRIV, PARENT1_Yes, EDUCATION_FLAG_BIN, MSTATUS_Yes, BLUEBOOK, OLDCLAIM, JOB_TYPE_FLAG_BIN, HOME_VAL, INCOME_FLAG_BIN, INCOME are statistically significant. 
- As for the statistically significant variables, CAR_USE_Commercial,REVOKED_Yes,URBANICITY_Rural have  the lowest p-value suggesting a strong association to the target variable.
- The logistic regression coefficients give the change in the log odds of the outcome for a one unit increase in the predictor variables. For Example:
  a. For change in KIDSDRIV variable when kids are in car, the log odds of having an accident **increases** by 0.4143 .\
  b. When the driver is a Single Parent the log odds of an accident **increases** by 0.4530\
  c. with change in marital status the log odds of accident **decreases** by 0.4939\
  d. If the CAR_TYPE is a MiniVan or a Panel Truck the log odds of accident **decreases** by 0.5573. \
- No. of iterations are 5 before lowest value of AIC was derived for this model.
...
#####  Interpretation for Second Model\

$\newline$

- The following variables have been used for classification - OLD_CLAIM, JOB_TYPE_FLAG_BIN, URBANICITY_Rural, KIDSDRIV, MVR_PTS, REVOKED_Yes.\
\

- lowest Cp value and Xerror occured on split 6
\
\
...
In this model we will be using different set of variables.  Also, we use the Linear Discriminant Analysis to train the model.

```{r, echo = FALSE, warning=FALSE, message=FALSE}

#ds <- select(DS_TARGET_FLAG_TRAIN, -AGE, -AGE_IMPUTE, -BLUEBOOK, -CAR_AGE_IMPUTE, -CAR_AGE_MISS, -CLM_FREQ, -HOME_VAL, -HOME_VAL_IMPUTE, -HOME_VAL_MISS, -INCOME, -INCOME_IMPUTE, -INCOME_MISS, -MVR_PTS, -OLDCLAIM, -TIF, -TRAVTIME, -YOJ, -YOJ_IMPUTE,-YOJ_MISS)

TF_Model3=lda(TARGET_FLAG~.,data=DS_TARGET_FLAG_TRAIN)
TF_Model3
```

#####  Interpretation for the Third Model  \

$\newline$

- The Classification boundary equation for Model 3 is below: \

$0.344 * KIDSDRIV - 0.0039 * AGE + 0.0103 * HOMEKIDS + 0.0111 * YOJ - 0 * INCOME - 0 * HOME_VAL + 0.009 * TRAVTIME - 0 * BLUEBOOK - 0.0403 * TIF - 0 * OLDCLAIM + 0.0736 * CLM_FREQ + 0.1203 * MVR_PTS + 0.0038 * CAR_AGE + 0.6333 * CAR_USE_Commercial - 0.3869 * MSTATUS_Yes + 0.4117 * PARENT1_Yes - 0.0264 * RED_CAR_yes + 0.9784 * REVOKED_Yes - 0.0133 * SEX_M - 1.5283 * URBANICITY_Rural - 0.0451 * YOJ_MISS - 0.067 * INCOME_MISS - 0.0196 * HOME_VAL_MISS + 0.1969 * CAR_AGE_MISS + 0.0314 * TIF_sin - 0.0278 * BLUEBOOK_sin + 0.0169 * AGE_sin - 0.4273 * CAR_TYPE_FLAG_BIN - 0.3207 * EDUCATION_FLAG_BIN + 0.3307 * JOB_TYPE_FLAG_BIN + 0.4485 * INCOME_FLAG_BIN + 0.0037 * YOJ_FLAG_BIN + 0.1432 * HOME_VAL_FLAG_BIN - 0.1942 * OLDCLAIM_FLAG_BIN - 0.1942 * CLM_FREQ_FLAG_BIN + 0.1028 * MVR_PTS_FLAG_BIN + 0.0748 * CAR_AGE_FLAG_BIN - 0.1087 * TRAVTIME_FLAG_BIN$

- From summary table, we also have the prior probability of of not having accident incident in data set is 0.7345594 and failure is 0.2654406.\

- Group means provides mean values for each variable with respect to target variable values 0 and 1.\

- LDA model assumes normality of the variables used in the model and there are some variables which are not completely normally distributed and have outliers that is impacting the result of this model.
...
kable(model_comparison,row.names = TRUE, caption = " Model 2 evaluation KPIs")
...
DS_TARGET_FLAG_VALID$M3_TARGET_FLAG <- as.integer(unlist(data.frame(predict(TF_Model3,newdata=DS_TARGET_FLAG_VALID))[,1]))

df_pre_train1<-as.data.frame(table(DS_TARGET_FLAG_VALID$M3_TARGET_FLAG==1,DS_TARGET_FLAG_VALID$TARGET_FLAG))
df_pre_train1$metrics <- paste(df_pre_train1$Var1,df_pre_train1$Var2, sep = '_') 
...
Following are the most relevant variables for the model: CAR_USE_Commercial, REVOKED_Yes, URBANICITY_Rural, CAR_TYPE_FLAG_BIN,TRAVTIME, OLDCLAIM_FLAG_BIN, MVR_PTS, TIF, KIDSDRIV, PARENT1_Yes, EDUCATION_FLAG_BIN, MSTATUS_Yes, BLUEBOOK, OLDCLAIM, JOB_TYPE_FLAG_BIN, HOME_VAL, INCOME_FLAG_BIN. \
\
...
exp(cbind(OR = coef(TF_Model1_ref), confint.default(TF_Model1_ref)))

```

The following points can be made for the important variables in the model: \

In keeping all other variables same, the odds of an accident increases as follow: 2.1451462 for per unit change in CAR_USE_Commercial, 2.7874004 per unit change in REVOKED_Yes, 0.1085252 for per unit change in URBANICITY_Rural, etc. Any value which is less than 1, it means that there is less chance of an event with the per unit increase of the variable. \
...
insure_train_crash <- select(insure_train_crash, -AGE_sin, -BLUEBOOK_sin, -CAR_AGE_FLAG_BIN, -CAR_TYPE_FLAG_BIN, -CLM_FREQ_FLAG_BIN, -EDUCATION_FLAG_BIN, -HOME_VAL_FLAG_BIN, -INCOME_FLAG_BIN, -JOB_TYPE_FLAG_BIN, -MVR_PTS_FLAG_BIN, -OLDCLAIM_FLAG_BIN, -TARGET_FLAG, -TIF_sin, -YOJ_FLAG_BIN)
...
In this model, we will be using the stadard lm modeling technique. We will use only those variables that were part of the original set of variables.  \

```{r, echo = FALSE, warning=FALSE, message=FALSE}

TA_Model1 <- lm(TARGET_AMT~.-TARGET_FLAG, data=na.omit(insure_orig))
summary(TA_Model1)
...
coef1<- data.frame('Coefficients'= TA_Model1$coefficients)
coef1 <- filter(coef1, !is.na(coefficients)) 
...
eval_ds$TARGET_FLAG_prob <- predict(TF_Model1_ref, type ="response", newdata=eval_ds)
eval_ds$TARGET_FLAG <- ifelse(eval_ds$TARGET_FLAG_prob>0.5, 1, 0)
table(eval_ds$TARGET_FLAG)
kable(eval_ds, caption="Outcome on evaluation data set")
...
# eval_ds$TARGET_AMT <- predict(TA_Model1, type ="response", newdata=eval_ds_TA)
# 
# 
# 
# kable(eval_ds_TA, caption="Outcome on evaluation data set")
