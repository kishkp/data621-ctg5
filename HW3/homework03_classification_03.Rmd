...
*** Curious It is clear from the table that most of the variables are having storng correlation with the target variable. \
...
Model 1- This model has been created using the availble variables in train data set with logit function GLM.\
Model 2- In this model step function is being used to enhance model 1 using train data.\
Model 3- Here , a new model has been created using GLM function and with tranformed variables.\
...
(iv) The logistic regression coefficients give the change in the log odds of the outcome for a one unit increase in the predictor variables.
  ...
  a. For every one unit change in nox, the log odds of crime rate above median value incremases by 56.02.\
  b. For a one unit increase in rad, the log odds of crime rate above median value incremases by 0.72.\
  c. For a one unit increase in dis, the log odds of crime rate above median value incremases by 0.82.\
...
(iii) The logistic regression coefficients give the change in the log odds of the outcome for a one unit increase in the predictor variables.
  ...
  a. For every one unit change in nox, the log odds of crime rate above median value incremases by 48.61.\
  b. For a one unit increase in rad, the log odds of crime rate above median value incremases by 0.79.\
  c. For a one unit increase in dis, the log odds of crime rate above median value incremases by 0.74.\
...
In this model Linear Discriminant Analysis function has been used with given set of variables in traning data.
...
In this model Linear Discriminant Analysis function has been used with transformed set of variables in traning data.\
...
(iii) One point to note here this model performs better than the previous one as outliers were taken care of in transformed set bringing more nomality to the model. But overall this model also perform less than logistics model. 
...
From the comparison table it can be seen model 1 is the best model with very high accuracy rate of 91.48%.But Model 2 is the best in terms of AUC value which is .9567 and Accuracy 90.42%. Model 1 also has close value of AUC socre of 95.44. But Model2 has lower AIC value of 164.85 where as the other one has AIC value 170.93.
Both model has same no of coefficient.  Based on the above data poiints model 1 is selected for slighly better acuracy. For final model following analysis has been carried out\
...
(ii)Keeping all other variables same odds of havin crime rate above median value increases following way-0.875 for per unit change in indus,2.50 per unit change in dis,1.74 for per unit change in rad,1.51 for per unit change in ptratio and 1.30 for per unit change in medv.
