...
```
Logit function has been used in building models in this assignment. Any transformation like sin,sqrt,nth etc for logit model brings additional complexity while doing the interpretation of the model. For that purpose any such transformation has not been used for the variables as normality is not a criteria for logit model. Following are some of the transformation of the variables that have been used here-
...
###??
$\newline$
(i)Classification boundary equation is following-
 -0.004 x zn+ 0.0281 x indus -0.055 x chas + 7.910 x nox + 0.165 X rm + 0.013 x age + 0.084 x dis + 0.102 x rad -0.001 x tax + 0.009 x ptratio  -0.0009 x black + 0.024 x lstat + 0.042 x medv =0


(ii) From summary table provides prior probability of outcome before the model is run.\
...
$\newline$
(i)Classification boundary equation is following-
 0.022 x indus -0.186 x chas + 7.970 x nox +  0.015 x age + 0.100 x rad -0.001 x tax - 0.014 x ptratio  -0.001 x black + 0.016 x lstat_new + 0.047 x medv_new -0.008 x rm_new -0.034 x dis_new -0.001 x zn_new =0
...
The following model selection strategy has been used for this assignment: 

(i) Compare accuracy & confusion matrix of the models.
(ii) Compare Precision, Sensitivity, Specificity, and F1 score.
...
\
(ii) nox variable has very high oddd ratio and reason is, in given data set there are records where for range of nox values there is only one outcome of target (either 0 or 1) as shown in the chrt below with vertical lines. That makes easy for prediction of target variable and increases the accuracy of the model. Outlier treatment for this variable will result in 50% reduction on train data set. For this assignment we have kept this variable.\
\
