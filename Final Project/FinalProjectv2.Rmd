...
-Model 3- This model will be created using RandomForests model. 
...
printcp(model2_ref) # display the results 
...
importance(model3) # importance of each predictor
...
# Predict result from the model 1

results1 <- predict(model1,newdata=DS_TARGET_FLAG_VALID,type='response')

DS_TARGET_FLAG_VALID$TARGET_FLAG<-results1

results1 <- ifelse(results1 > 0.5,1,0)

misClasificError <- mean(results1 != DS_TARGET_FLAG_VALID$y)
...
myRoc <- roc(DS_TARGET_FLAG_VALID$y~DS_TARGET_FLAG_VALID$TARGET_FLAG) 
...
results2 <- predict(model2_ref,newdata=DS_TARGET_FLAG_VALID)[,2]

DS_TARGET_FLAG_VALID$TARGET_FLAG2<- predict(model2_ref,newdata=DS_TARGET_FLAG_VALID)[,2]

results2 <- ifelse(results2 > 0.5,1,0)

misClasificError <- mean(results2 != DS_TARGET_FLAG_VALID$y)
...
myRoc2 <- roc(DS_TARGET_FLAG_VALID$y~DS_TARGET_FLAG_VALID$TARGET_FLAG2, DS_TARGET_FLAG_VALID)
...
results3 <- as.numeric(predict(model3,newdata=DS_TARGET_FLAG_VALID,type='response'))

DS_TARGET_FLAG_VALID$TARGET_FLAG3<-results3


misClasificError <- as.numeric(mean(results3 != DS_TARGET_FLAG_VALID$y))
print(paste('Accuracy',1-misClasificError))


# specificty vs sensitivity

pr <- prediction(results3, DS_TARGET_FLAG_VALID$y)
...
myRoc <- roc(DS_TARGET_FLAG_VALID$y~DS_TARGET_FLAG_VALID$TARGET_FLAG3) 
plot(myRoc, main="ROC Curve for Classification data") 
