...
legend("top", colnames(model3$err.rate),col=1:4,cex=0.8,fill=1:4)
...
results1 <- predict(model1_update,newdata=DS_TARGET_FLAG_VALID,type='response')
...
df_pre_train1<-as.data.frame(table(DS_TARGET_FLAG_VALID$TARGET_FLAG>0.5,DS_TARGET_FLAG_VALID$y))
df_pre_train1$metrics <- paste(df_pre_train1$Var1,df_pre_train1$Var2, sep = '_') 
model_comparison[1,]<-Eval(df_pre_train1)
DS_TARGET_FLAG_VALID$TARGET_FLAG<-as.numeric(ifelse(DS_TARGET_FLAG_VALID$TARGET_FLAG >0.5,1,0))
model_comparison[1,c("AUC")]<-c(auc(DS_TARGET_FLAG_VALID$y,DS_TARGET_FLAG_VALID$TARGET_FLAG))
...
DS_TARGET_FLAG_VALID$results2 <- ifelse(results2 > 0.5,1,0)
...
# ROC curve
levels(DS_TARGET_FLAG_VALID$TARGET_FLAG3)<-c(0,1)
myRoc <- roc(DS_TARGET_FLAG_VALID$y~DS_TARGET_FLAG_VALID$TARGET_FLAG3,DS_TARGET_FLAG_VALID) 
plot(myRoc, main="ROC Curve for Classification data") 
