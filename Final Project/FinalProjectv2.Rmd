...
Logistic regression (logit model) is used to model dichotomous outcome variables. In the logit model the log odds of the outcome is modeled as a linear combination of the predictor variables.
...
#colnames(bank_test2)[15]<-c("job_blue_collar")
#colnames(DS_TARGET_FLAG_TRAIN)[20]<-c("job_self_employed")
...
model1 <- glm(y ~.,family=binomial,data=na.omit(DS_TARGET_FLAG_TRAIN))
summary(model1)

#anova(model1, test="Chisq")
model1_update<-glm(y ~.-job_student-marital_unknown-education_tertiary-education_tertiary-default_yes-housing_unknown-loan_yes-loan_unknown-contact_cellular-month_sep-day_of_week_fri-poutcome_success,family=binomial,data=na.omit(DS_TARGET_FLAG_TRAIN))
...
t1<-cv.glm(DS_TARGET_FLAG_TRAIN, model1_update, K = 10)$call
cv.glm(data = DS_TARGET_FLAG_TRAIN, glmfit = model1_update, K = 5)
...
model2 <- rpart(y~., data=na.omit(DS_TARGET_FLAG_TRAIN), method = "class")
...
model4 <- randomForest(as.factor(y) ~ .,data=DS_TARGET_FLAG_TRAIN,importance=TRUE, ntree=100)
...
DS_TARGET_FLAG_VALID$TARGET_FLAG1<-predict(model1_update,newdata=na.omit(DS_TARGET_FLAG_VALID),type='response')
...
m <- model1_update<-glm(y ~.-job_student-marital_unknown-education_tertiary-education_tertiary-default_yes-housing_unknown-loan_yes-loan_unknown-contact_cellular-month_sep-day_of_week_fri-poutcome_success,family=binomial,data=na.omit(DS_TARGET_FLAG_TRAIN))
hoslem.test(model1_update$y, fitted(m))
```


3. Hosmer-Lemeshow goodness-of-fit (GOF) tests help you decide whether your model is correctly specified. They produce a p-value-if it's low (say, below .05), you reject the model. If it's high, then the model passes the test.As per the Hosmer-Lemeshow test is a statistical test for goodness of fit for logistic regression models this is not good fit for regression with low p value p-value < 2.2e-16.


## Results from CRT Model
...
results3 <- ifelse(as.numeric(predict(model4,newdata=na.omit(DS_TARGET_FLAG_VALID),type='response'))==1,0,1)
...
myRoc1 <- roc(DS_TARGET_FLAG_VALID$TARGET_FLAG1,DS_TARGET_FLAG_VALID$y) 


# Area under curve model 2
myRoc2 <- roc(DS_TARGET_FLAG_VALID$TARGET_FLAG2,DS_TARGET_FLAG_VALID$y)


# Area under curve model 3

myRoc3<- roc(DS_TARGET_FLAG_VALID$TARGET_FLAG3,DS_TARGET_FLAG_VALID$y) 
