...
kable(modelvars, caption = 'Model Variables')
...
c1<- summary(poismod1)
kable(c1$coefficients, caption = 'Model 1 Poisson Original Data')
```
...
c2<- summary(fm_qpois)
kable(c2$coefficients, caption = 'Model 2 Quasi-Poisson Original Data')
```
...
kable(c3$coefficients, caption = 'Model 3, Zero Inflation Poisson Original Data')
...
kable(c4$coefficients, caption = 'Model 4, Poisson Transformed Data')
...
c5<- summary(mod2qpois)


kable(c5$coefficients, caption = 'Model 5 Quasi-Poisson Transformed Data')
```
...
kable(c6$coefficients, caption = 'Model 6, Zero Inflation Poisson Transformed Data')
```
...
c7<- summary(nbmod3)
kable(c7$coefficients, caption = 'Model 7, Negative binomial Original Data')        
...
From the summary output, we have the following estimated model:
$$
 \hat y = e^{B_0x_0+B_1x_1+B_2x_2+ B_3x_3+B_4x_4+ B_5x_5+B_6x_6+ B_7x_7+B_8x_8+ B_9x_9+B_{10}x_{10}+B_{11}x_{11}+B_{12}x_{12}+ B_{13}x_{13}+B_{14}x_{14}}
$$
...
c8<- summary(nbmod3zip)

kable(c8$coefficients, caption = 'Model 8, Zero Inflation Negative binomial Original Data')        
...
kable(c9$coefficients, caption = 'Model 9, Negative binomial Transformed Data')        
...
***Note***
As per the below table, even for transformed data, it is worth noting that the classical Poisson Coefficients are similar to that of the Negative Binomial's for the same reason as was the case for original data. 
Please refer to Section: 5.3.1.1 "Negative Binomial vs Poisson Coefficients" for more details. \
...
kable(c10$coefficients, caption = 'Model 10, Zero Inflation Negative binomial Transformed Data')        
```
...
kable(c11$coefficients, caption = 'Model 11, Linear Model Original Data')        
...
kable(c12$coefficients, caption = 'Model 11, Linear Model Transformed Data')        
...
t<- x[1:6,]
t2<- t(t)
kable(t2,  caption="Transposed Model Output / Results")
...
```{r code=readLines(knitr::purl('https://raw.githubusercontent.com/kishkp/data621-ctg5/master/HW5/hw05_Group05.Rmd', documentation = 0)), eval = FALSE}
