...
# Compae the four models build above to identify the best model to be used for prediction
...
# from AIC values it appears model "step1" & "step 4" are comparativey better model of the pack. Now evaluate the model by using adjusted R^2 values

summary(step1);summary(step4)

# Based on adjusted R^2 value for Models "step2" and "step4" we can conslude model "step4" is the best one with R^2 value .5585. Hence this model can explain 55.55% of variablity in the model. 
...
# Based on the plot it appears residula distrubution is normal.
...
# Distribution of residual is random
...
# Analysis of collinearity and enhanceof of model

```{r collonearity of model}
...
# Based on that VIF values delete the variable "TEAM_PITCHING_BB " from model and evaluate model performance
...
summary(step4_up)
# There was no significant change in adjusted R^2 values

sqrt(vif(step4_up))

# Based on that VIF values delete the variable "TEAM_BATTING_SO " from model and evaluate model performance
...
# Based on that VIF values delete the variable " Strikeout_R " from model and evaluate model performance
...
# Based on that VIF values delete the variable " HomeRuns_R " from model and evaluate model performance

step4_up<-lm(TARGET_WINS ~TEAM_BATTING_H + TEAM_BATTING_BB+ TEAM_BATTING_HBP+ TEAM_PITCHING_HR + TEAM_PITCHING_SO + TEAM_FIELDING_E + TEAM_FIELDING_DP + TEAM_BATTING_BB_SIN + TEAM_BASERUN_SB_SIN + TEAM_FIELDING_DP_SIN + Walks_R  ,data=moneyball2 )

summary(step4_up)
# There was no significant change in adjusted R^2 values

sqrt(vif(step4_up))
#Now VIF values are under 2 for all the variables and this variable looks like the best one
